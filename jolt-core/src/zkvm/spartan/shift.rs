...
impl<F: JoltField> ShiftSumcheck<F> {
    ...
    pub fn new_prover<ProofTranscript: Transcript, PCS: CommitmentScheme<Field = F>>(
        ...
        assert_eq!(r_cycle.len(), key.num_steps.ilog2() as usize);

        let (r_product, _) = state_manager.get_virtual_polynomial_opening(
            VirtualPolynomial::NextIsNoop,
            SumcheckId::ProductVirtualization,
        );
        assert_eq!(r_product.len(), key.num_steps.ilog2() as usize);
...
impl<F: JoltField, T: Transcript> SumcheckInstance<F, T> for ShiftSumcheck<F> {
    ...
    fn compute_prover_message(&mut self, _round: usize, _previous_claim: F) -> Vec<F> {
        println!("PREV CL: {}", _previous_claim);
        let res = self.prover.as_ref().unwrap().compute_prover_message();
        println!("EVAL IS: {}", res[0]);
        res
    }
...
///
/// Performs prefix-suffix sumcheck <https://eprint.iacr.org/2025/611.pdf> (Appendix A).
    ...
    fn gen(
        ...
        for x0 in 0..1 << prefix_n_vars {
            for x1 in 0..1 << suffix_n_vars {
                ...
                Q_0_for_r_cycle[x0] += v * suffix_0_for_r_cycle.get_bound_coeff(x1);
                Q_1_for_r_cycle[x0] += v * suffix_1_for_r_cycle.get_bound_coeff(x1);

                // q += suffix * (1 - is_noop)
                if !is_noop {
                    Q_0_for_r_prod[x0] += suffix_0_for_r_prod.get_bound_coeff(x1);
                    Q_1_for_r_prod[x0] += suffix_1_for_r_prod.get_bound_coeff(x1);
                }
            }
        }
        ...
        let prefix_suffix_pairs = vec![
            (P_0_for_r_cycle, Q_0_for_r_cycle.into()),
            (P_1_for_r_cycle, Q_1_for_r_cycle.into()),
            (P_0_for_r_prod, Q_0_for_r_prod.into()),
            (P_1_for_r_prod, Q_1_for_r_prod.into()),
        ];
...
impl<F: JoltField> Phase2Prover<F> {
    fn gen(
        ...
        let prefix_0_eval = prefix_0.evaluate(&r_prefix.r);
        let prefix_1_eval = prefix_1.evaluate(&r_prefix.r);
        let eq_plus_one_r_cycle: MultilinearPolynomial<F> = (0..suffix_0.len())
            .map(|i| {
                prefix_0_eval * suffix_0.get_bound_coeff(i)
                    + prefix_1_eval * suffix_1.get_bound_coeff(i)
            })
            .collect::<Vec<F>>()
            .into();
        ...
        // Generate bound MLEs:
        //   UnexpandedPc(r_prefix, j), Pc(r_prefix, j), ...
        ...
        (
            ...
            .for_each(
                |(
                    ...
                    let mut local_unexpanded_pc_eval = F::zero();
                    let mut local_pc_eval = F::zero();
                    let mut local_is_virtual_eval = F::zero();
                    let mut local_is_first_in_sequence_eval = F::zero();
                    let mut local_is_noop_eval = F::zero();
                    for (i, cycle) in trace_chunk.iter().enumerate() {
                        ...
                        local_unexpanded_pc_eval += eq_eval.mul_u64(unexpanded_pc);
                        local_pc_eval += eq_eval.mul_u64(pc);
                        if is_virtual {
                            local_is_virtual_eval += eq_eval;
                        }
                        if is_first_in_sequence {
                            local_is_first_in_sequence_eval += eq_eval;
                        }
                        if is_noop {
                            local_is_noop_eval += eq_eval;
                        }
                    }
                    *unexpanded_pc_eval = local_unexpanded_pc_eval;
                    *pc_eval = local_pc_eval;
                    *is_virtual_eval = local_is_virtual_eval;
                    *is_first_in_sequence_eval = local_is_first_in_sequence_eval;
                    *is_noop_eval = local_is_noop_eval;
                },
            );
...
struct EqPlusOnePrefixSuffixPoly<F: JoltField> {
    ...
    prefix_0: MultilinearPolynomial<F>,
    // Evals of `eq(r_hi, j)` for all j in the hypercube.
    suffix_0: MultilinearPolynomial<F>,
    // Evals of `is_max(r_lo) * is_min(j)` for all j in the hypercube.
    // Where `is_max(x) = eq((1)^n, x)`, `is_min(x) = eq((0)^n, x)`.
    // TODO: This is non-zero in 1 position. Maybe more optimal to take advantage of this.
    prefix_1: MultilinearPolynomial<F>,
    // Evals of `eq+1(r_hi, j)` for all j in the hypercube.
    suffix_1: MultilinearPolynomial<F>,
}
...
impl<F: JoltField> EqPlusOnePrefixSuffixPoly<F> {
    fn new(r: &OpeningPoint<BIG_ENDIAN, F>) -> Self {
        ...
        Self {
            prefix_0: eq_plus_one_evals::<F>(&r_lo.r).into(),
            suffix_0: EqPolynomial::evals(&r_hi.r).into(),
            prefix_1: prefix_1_evals.into(),
            suffix_1: eq_plus_one_evals::<F>(&r_hi.r).into(),
        }
    }
}

/// Evaluates `eq+1(r, j)` for all `j` in the hypercube.
///
/// `r` should be in big-endian.
fn eq_plus_one_evals<F: JoltField>(r: &[F::Challenge]) -> Vec<F> {
    // let mut eq_evals = EqPolynomial::evals(r);
    // eq_evals.rotate_left(1);
    // *eq_evals.last_mut().unwrap() = F::zero();
    // eq_evals
...
mod tests {
    ...
    use super::{eq_plus_one_evals, EqPlusOnePrefixSuffixPoly};

    #[test]
    fn test_eq_prefix_suffix() {
        let r = OpeningPoint::<BIG_ENDIAN, Fr>::new([9, 2, 3, 7].map(<_>::into).to_vec());
        let eq_plus_one_gt = MultilinearPolynomial::from(eq_plus_one_evals::<Fr>(&r.r));
        ...
        assert_eq!(
            prefix_0.evaluate(&r_prime_lo.r) * suffix_0.evaluate(&r_prime_hi.r)
                + prefix_1.evaluate(&r_prime_lo.r) * suffix_1.evaluate(&r_prime_hi.r),
